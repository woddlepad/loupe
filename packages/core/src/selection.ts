...
const C = {
  ...
  libBtn:
    "h-12 px-2.5 inline-flex items-center gap-1.5 rounded-md border border-dashed border-loupe-line-strong text-loupe-faint hover:text-loupe-fg hover:border-loupe-accent/50 text-[11px] cursor-pointer transition-colors",
  pickerWrap: "basis-full relative mt-1",
  picker:
    "rounded-lg bg-loupe-panel border border-loupe-line max-h-72 overflow-hidden shadow-2xl shadow-black/50",
  pickerSearch:
    "w-full box-border rounded-md bg-loupe-bg/80 border border-loupe-line text-loupe-fg text-[12px] px-2 py-1.5 outline-none transition-colors focus:border-loupe-accent/60 placeholder:text-loupe-faint",
  pickerList: "max-h-56 overflow-y-auto p-2 space-y-2",
  pickerGroupButton:
  ...
  pickerGrid: "grid grid-cols-2 gap-2 pt-1",
  pickCell:
    "overflow-hidden rounded-lg border border-loupe-line bg-loupe-bg/70 text-left hover:border-loupe-accent/50 cursor-pointer p-0 transition-colors",
  pickImgWrap: "aspect-[16/9] border-b border-loupe-line bg-black/40",
  pickText: "p-2 text-[11px] text-loupe-fg leading-snug line-clamp-2",
...
export class LoupeOverlay {
  ...
  private openLibraryPicker(row: HTMLElement, before: HTMLElement): void {
    ...
    const searchWrap = el("div", { class: "p-2 border-b border-loupe-line" });
    const search = el("input", {
    ...
    const render = () => {
      ...
      if (this.opts.library.length === 0) {
        list.append(el("div", { class: "p-3 text-loupe-faint text-[12px] text-center" }, "no references in the library yet"));
        return;
      }
      if (groups.length === 0) {
        list.append(el("div", { class: "p-3 text-loupe-faint text-[12px] text-center" }, "no matches"));
        return;
      }
      for (const [domain, items] of groups) {
        ...
        if (!isCollapsed) {
          ...
          for (const item of items) {
            ...
            cell.append(imageWrap, el("div", { class: C.pickText }, item.caption || item.url || item.id));
            cell.addEventListener("click", async () => {
              wrap.remove();
              const dataUrl = await this.opts.resolveLibraryImage(item.id);
              if (dataUrl) this.addRefDataUrl(dataUrl, row, before);
            });
            grid.append(cell);
          }
          section.append(grid);
